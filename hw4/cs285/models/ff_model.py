...
class FFModel(nn.Module, BaseModel):
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        torch_data_statistics = {k: ptu.from_numpy(v) for k, v in data_statistics.items()}
        target = normalize(next_observations - observations, torch_data_statistics['delta_mean'], torch_data_statistics['delta_std'])
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.
        delta_pred_normalized = self.forward(observations,
                                             actions,
                                             torch_data_statistics['obs_mean'],
                                             torch_data_statistics['obs_std'],
                                             torch_data_statistics['acs_mean'],
                                             torch_data_statistics['acs_std'],
                                             torch_data_statistics['delta_mean'],
                                             torch_data_statistics['delta_std'])[1]
